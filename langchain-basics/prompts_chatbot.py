    ...
    def get_response(self) -> str:
        chat_template = ChatPromptTemplate(
            messages=[
                (
                    "system",
                    f"You are an assistant of character: {self.character}. You can answer questions and help with tasks based on the chat history.",
                ),
...
character = st.selectbox(
    "Select Character", ["Serious", "Hip hop artist", "Friend", "Comedian"]
)
...
if chat_input:
    response = ModelResponse(
        chat_input, st.session_state.history, character
    ).get_response()
